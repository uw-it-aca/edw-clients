    ...
    def get_enrolled_students_df(self, sis_term_id, filters=None):
        ...
        if filters and filters.get("searchFilter"):
            ...
            if filter_type == "student-number":
                filters_list.append(
                    f"AND StudentNumber LIKE '%{filter_text}%'")
            elif filter_type == "student-name":
                filters_list.append(
                    f"AND UPPER(enr.StudentName) LIKE "
                    f"UPPER('%{filter_text}%')")
            elif filter_type == "student-email":
                filters_list.append(
                    f"AND UPPER(enr.StudentEmail) LIKE "
                    f"UPPER('%{filter_text}%')")
        ...
        query = (
            ...
            WITH student_info AS (
                ...
                WHERE AcademicYrQtr = '{yrq}' AND dm.MajorFullName IS NOT NULL {' '.join(filters_list)}
            )
            SELECT
                StudentNumber,
                UWNetID,
